...
class DefensiveReactiveAgent12() : PlanetWarsPlayer() { // New agent defined as subclass of PlanetWarsPlayer()

    /*
    v1.2 - More accurately calculate planet ship production when launching attacks and defending. Added better
    calculation for ships sent.
    ...
    */
    ...
    private val NEUTRAL_DISTANCE_WEIGHT: Double = 0.9
    private val ENEMY_DISTANCE_WEIGHT: Double = 0.6
    private val SMALL_SOURCE_TICK_DIVISOR: Double = 5.0
    private val PERCENTAGE_TOTAL_TO_SEND: Double = 0.70
    private val PARTIAL_ATTACK_PENALTY: Double = 1.5
    ...
    override fun getAction(gameState: GameState): Action { //Override getAction from original class, pass gameState in and return action.
        ...
        // from being captured.
        val defendAction = defendStrategy(myPlanets.sortedBy{it.nShips + planetInfo.getOrElse(it.id) { IncomingShipsInfo() }.netShips }, planetInfo, gameState, mySourcePlanets)
    ...
    private fun attackPlanetStrategy(enemyPlanets: List<Planet>, gameState: GameState, planetInfo: MutableMap<Int, IncomingShipsInfo>, mySourcePlanets: List<Planet>): Action {
        // Create list of planets we have calculated the ships generated for already
        val growthRateCalculatedPlanets = HashMap<Planet, Double>()
        // For all enemy planets that are getting captured, calculate the planets' generated ships and add to the total.
        for (p in enemyPlanets) {
            // If we think the planet will be captured, add ships generated to truly calculate the current future total.
            if (planetInfo.getOrElse(p.id) {IncomingShipsInfo()}.netShips - p.nShips >= 0) {
                val generated: Double = determineShipsGenerated(gameState, p, planetInfo)
                planetInfo[p.id]!!.netShips += generated
                growthRateCalculatedPlanets[p] = generated/p.growthRate
            }
        }
        ...
        var act = attackPlanetWithSource(planetsToAttack, optimalSources, planetInfo, growthRateCalculatedPlanets)
    ...
    // in an action.
    private fun attackPlanetWithSource(targetList: List<Planet>, sourceList: List<Planet>, planetInfo: MutableMap<Int, IncomingShipsInfo>, grCalcPlanets: HashMap<Planet, Double>): Action {
        ...
        for (s in sourceList) {
            ...
            for (t in targetList) {
                ...
                var incomingNet = planetInfo.getOrElse(t.id) {IncomingShipsInfo()}.netShips - t.nShips
                val checkDist = getDist(s,t)

                // If the planet's growth rate was already accounted for to check for capture
                if (grCalcPlanets.containsKey(t)) {
                    // Add ships generated for the extra time after the growth rate was calculated for.
                    val timeDifference: Double = getPlanetTravelTime(s, t) - grCalcPlanets[t]!!
                    if (timeDifference > 0) {
                        incomingNet -= (timeDifference * t.growthRate)
                    }
                }
                // If the planet's growth rate hasn't been accounted for, utilise it based on time for source to reach target.
                else if (t.owner != Player.Neutral) {
                    incomingNet -= (getPlanetTravelTime(s, t) * t.growthRate)
                }
                ...
                var combinedScore = tScore + (sScore * 0.5)
                // If new score is better than the stored one, it is the new best action.
                val shipsNeeded = -incomingNet + 1.0

                if (s.nShips < shipsNeeded && t.owner != Player.Neutral) {
                    combinedScore *= PARTIAL_ATTACK_PENALTY
                }
                if (combinedScore < bestScore) {
                    source = s
                    target = t
                    bestScore = combinedScore
                    bestShipsNeeded = shipsNeeded
                }
            }
        }
        return Action(player, source.id, target.id, getShipsToSend(bestShipsNeeded, source))
    }
    ...
    private fun getShipsToSend(needed: Double, source: Planet): Double {
        ...
        return (source.nShips * PERCENTAGE_TOTAL_TO_SEND)
    }
    ...
    // Calculate the travel time for a planet to launch a transporter to reach the target planet.
    private fun getPlanetTravelTime(source: Planet, target: Planet): Double {
        return (getDist(source, target) / params.transporterSpeed)
    }
    ...
    override fun getAgentType(): String {
        return "Defensive Reactive Agent 1.2"
    }
}

fun main() {
    val agent = DefensiveReactiveAgent12()
